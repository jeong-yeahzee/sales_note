...
function create_trigger_query(){
  ...
  const trigger_calc = `
    ...
    BEGIN
        UPDATE TBL_SHOP
        SET TOTAL_SALES_OUTSTANDING = TOTAL_SALES_OUTSTANDING - (NEW.PAYMENT_AMOUNT + NEW.DISCOUNT_AMOUNT + NEW.ADMIN_AMOUNT)
        WHERE SHOP_NO = NEW.SHOP_NO;
    END;
    ...
    BEGIN
        UPDATE TBL_SHOP
        SET TOTAL_SALES_OUTSTANDING = TOTAL_SALES_OUTSTANDING - ((NEW.PAYMENT_AMOUNT - OLD.PAYMENT_AMOUNT) + (NEW.DISCOUNT_AMOUNT - OLD.DISCOUNT_AMOUNT) + (NEW.ADMIN_AMOUNT - OLD.ADMIN_AMOUNT))
        WHERE SHOP_NO = NEW.SHOP_NO;
    END;
